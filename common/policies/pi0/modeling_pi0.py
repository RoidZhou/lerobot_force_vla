...
class PI0Policy(PreTrainedPolicy):
    ...
    def prepare_future_effort(self, batch):
        ...
        future_effort = pad_sequence(future_effort, self.config.n_action_steps)
...
class PI0FlowMatching(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.config.force_prediction_enabled and self.config.force_prediction_expert_enabled:
            self.force_prediction_expert = copy.deepcopy(self.paligemma_with_expert.gemma_expert)
        else:
        ...
        if self.force_prediction_expert is not None:
            ...
            self.force_prediction_effort_proj_in = nn.Linear(
                prediction_effort_in_dim, 2 * self.config.proj_width
            )
            self.force_prediction_effort_proj_out = nn.Linear(2 * self.config.proj_width, self.config.proj_width)
        else:
        ...
        if self.config.force_prediction_enabled:
            self.force_pred_head = nn.Linear(self.config.proj_width, self.config.effort_dim)
        else:
    ...
    def embed_force_prediction_suffix(self, state, noisy_actions, timestep, effort: torch.Tensor = None):
        """Embed state, force-history tokens, noisy actions, and time for the prediction expert."""
        embs = []
        pad_masks = []
        att_masks = []
        ...
        action_emb = self.action_in_proj(noisy_actions.detach())
    ...
    def forward(
        ...
        force_refine_losses, force_pred = force_refine_losses
        force_prediction_loss = None
    ...
    def forward_force_refine(
        ...
        if self.config.force_shared_attention_enabled:
            ...
            refine_hidden = force_suffix_out[:, -self.config.n_action_steps :]
        ...
        suffix_out = suffix_out.to(dtype=torch.float32)
        refine_hidden = suffix_out[:, -self.config.n_action_steps :]
        force_v_t = self.force_refine_out_proj(refine_hidden)
        force_refine_losses = F.mse_loss(u_t, force_v_t, reduction="none")
        if self.force_prediction_expert is not None:
            ...
            force_pred = self.predict_force_from_suffix(suffix_out, detach_hidden=True)
        if return_force_pred:
            return force_refine_losses, force_pred
        return force_refine_losses
    ...
    def pool_force_hidden(self, force_suffix_out: Tensor) -> Tensor:
        n_force_tokens = force_suffix_out.shape[1] - self.config.n_action_steps
        if n_force_tokens > 0:
            return force_suffix_out[:, :n_force_tokens].mean(dim=1)
        return force_suffix_out[:, -self.config.n_action_steps :].mean(dim=1)
    ...
    def forward_force_from_action_cache(
        ...
        force_suffix_embs, force_suffix_pad_masks, force_suffix_att_masks = self.embed_suffix(
        ...
        (_, force_suffix_out), _ = self.paligemma_with_expert.forward(
            attention_mask=att_2d_masks,
            position_ids=position_ids,
            past_key_values=action_context_cache,
    ...
    def denoise_step(
        ...
        if force_refine and self.config.force_shared_attention_enabled:
            ...
            suffix_out = force_suffix_out[:, -self.config.n_action_steps :]
